...
try {
	var config_file = fs.readFileSync('/etc/swarm-gateway/influxdb-lab11-meta.conf', 'utf-8');
	...
	if (config.host == undefined || config.host == '' ||
	    ...
	    config.wiki_username == undefined || config.wiki_username == '' ||
	    config.wiki_password == undefined || config.wiki_password == '' ||
	    config.wiki_url == undefined || config.wiki_url == '') {
		...
		if (pieces.length >= 2) {
			...
			if (device_id_index > -1) {
				...
				if (device_id in _devices) {
					...
					if (_devices[device_id].location.length > 0) {
						addon += ',location=' + sanitize_tag(_devices[device_id].location);
					}
					if (_devices[device_id].description.length > 0) {
						addon += ',description=' + sanitize_tag(_devices[device_id].description);
					}
					if (_devices[device_id].notes.length > 0) {
						addon += ',notes=' + sanitize_tag(_devices[device_id].notes);
					}
